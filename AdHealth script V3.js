function adHealthCheck(spreadsheetUrl, consultantName) {
  ...
  var data = labelsSheet.getRange(2, 1, lastRow - 1, 3).getValues(); // A2:C

  for (var i = 0; i < data.length; i++) {
    var rowName = data[i][0]; // kolom A
    var rowLabel = data[i][1]; // kolom B
    var rowMail = data[i][2]; // kolom C

    if (rowName === consultantName) {
      accountLabel = rowLabel;
      mailadres = rowMail;
      break;
    }
  }

  if (!accountLabel || !mailadres) {
    Logger.log(
      'Consultant "' +
        consultantName +
        '" niet gevonden in de Labels sheet. Script gestopt.',
    );
    return;
  }

  if (weekDay == "Monday" && hour == 23) {
    logAccounts(todayString);
  }
  ...
  if (!newSetup && mailadres.length > 0 && accountLabel.length > 0) {
    ...
    // Send email with foundings of last run in case it timed out
    if (hour > 6) {
      sendEmail(mailadres);
    }
    ...
    Logger.log("All checks were finished for this round.");
    return;
  }
  ...
  function runChecks(checkFrequency) {
    ...
    // Get all account IDs from column A starting at row 3.
    var accountIds = settingsSheet
      .getRange("A3:A")
      .getValues()
      .flat()
      .filter(Boolean);

    // Get all account names from column B starting at row 3.
    var accountNames = settingsSheet
      .getRange("B3:B")
      .getValues()
      .flat()
      .filter(Boolean);
    ...
    var checkNames = settingsSheet.getRange("E2:Z2").getValues()[0];
    ...
    var checkFrequencies = settingsSheet.getRange("E3:Z").getValues();

    // Iterate over each account and do checks.
    for (var i = 0; i < accountIds.length; i++) {
      var rowNumber = i + 3;
      var columnCorrection = 5;
      var accountId = accountIds[i];
      var accountname = accountNames[i];
      var clientConsultants = settingsSheet.getRange(rowNumber, 3).getValue();
      var emailWhitelistInput = settingsSheet
        .getRange(rowNumber, 4)
        .getValue()
        .toLowerCase();
      var emailWhitelist =
        emailWhitelistInput +
        ",netprofiler.nl,leadscope.nl,clickvalue.nl,channable.com";

      // Controleer of dit account bij deze consultant hoort
      if (clientConsultants.indexOf(consultantName) === -1) {
        //Logger.log(checkFrequency + " -- " & accountname & " -- Consultant " + consultantName + " is niet aan dit account gekoppeld.");
        continue;
      }
      ...
      for (var j = 0; j < checkNames.length; j++) {
        ...
        switch (checkName) {
          ...
          case "Budget monitoring":
            var budgetPosition =
              checkNames.indexOf("Monthly budget") + columnCorrection;
            var accountBudget = settingsSheet
              .getRange(rowNumber, budgetPosition)
              .getValue();
            var positionBudgetFrequecy =
              checkNames.indexOf("Budget monitoring") + columnCorrection;
            var budgetMonitorFrequecy = settingsSheet
              .getRange(rowNumber, positionBudgetFrequecy)
              .getValue();
            if (accountBudget && budgetMonitorFrequecy === "Never") {
              settingsSheet
                .getRange(rowNumber, positionBudgetFrequecy)
                .setValue("Daily");
              Logger.log("Set budget monitoring frequency to Daily");
              var frequency = "Daily";
            }
            break;
        }

        var logSheet = getOrCreateLogSheet(checkFrequency, consultantName);

        // Check if the script for this account ID has already run in the period. -----------------------------------------------------------------
        var alreadyRun = false;
        if (frequency === checkFrequency) {
          var logData = logSheet.getDataRange().getValues();
          alreadyRun = logData.some(
            (row) => row[0] === checkName && row[1] === accountId,
          );
        }

        if (!alreadyRun && frequency == checkFrequency && !timeOut) {
          Logger.log(
            checkFrequency + " -- " + accountname + " -- " + checkName,
          );
          try {
            switch (checkName) {
              case "Account down":
              ...
              case "Target ROAS check":
                var positionMinTROAS =
                  checkNames.indexOf("Minimum target ROAS amount") +
                  columnCorrection;
                var minTRoas = settingsSheet
                  .getRange(rowNumber, positionMinTROAS)
                  .getValue();
                tROASCheck(
                  frequency,
                  accountNames[i],
              ...
              case "Search term CPC check":
                var positionMaxSearchTermCpc =
                  checkNames.indexOf("Search term CPC amount") +
                  columnCorrection;
                var maxSearchTermCpc = settingsSheet
                  .getRange(rowNumber, positionMaxSearchTermCpc)
                  .getValue();
                maxSearchTermCpcCheck(
                  frequency,
                  accountNames[i],
                  logSheet,
                  mailbodySheet,
                ...
                break;
              case "Disapproved products":
                var merchantCenterIdPosition =
                  checkNames.indexOf("Merchant Center ID") + columnCorrection;
                var merchantCenterId = settingsSheet
                  .getRange(rowNumber, merchantCenterIdPosition)
                  .getValue();
                var productThresholdPosition =
                  checkNames.indexOf("Acceptable % of disapproved products") +
                  columnCorrection;
                var productThreshold = settingsSheet
                  .getRange(rowNumber, productThresholdPosition)
                  .getValue();
                ...
                break;
              case "Budget monitoring":
                var campaignTypePosition =
                  checkNames.indexOf("Only Shopping/ Pmax/Search") +
                  columnCorrection;
                var campaignType = settingsSheet
                  .getRange(rowNumber, campaignTypePosition)
                  .getValue();
                var limitTypePosition =
                  checkNames.indexOf("Upper limit only") + columnCorrection;
                var limitType = settingsSheet
                  .getRange(rowNumber, limitTypePosition)
                  .getValue();
                budgetMonitoring(
                  frequency,
                  accountNames[i],
              ...
              case "Conversion recency check":
                var daysToCheckPosition =
                  checkNames.indexOf("Days since last conversion") +
                  columnCorrection;
                var daysToCheck = settingsSheet
                  .getRange(rowNumber, daysToCheckPosition)
                  .getValue();
              ...
              case "Lin Rodnitzky Ratio":
                ...
                break;
              case "Merchant Center ID check":
                var positionMerchantCenterId =
                  checkNames.indexOf("Merchant Center ID") + columnCorrection;
                var existingMerchantCenterId = settingsSheet
                  .getRange(rowNumber, positionMerchantCenterId)
                  .getValue();
                if (!existingMerchantCenterId) {
                  merchantCenterIdCheck(
                    frequency,
                    accountNames[i],
                    logSheet,
                    mailbodySheet,
  ...
  function accountDown(
    ...
    var accountSelector = MccApp.accounts().withCondition(
      'customer_client.id = "' + accountIdClean + '"',
    );
  ...
  function checkActivityYesterday(
    frequency,
    accountName,
    logSheet,
    mailbodySheet,
    ...
    if (rows.hasNext()) {
      Logger.log(
        frequency +
          " -- " +
          accountName +
          " -- Account down -- The Account seems to be active yesterday.",
      );
    } else {
      Logger.log(
        frequency +
          " -- " +
          accountName +
          " -- The Account seems to be inactive yesterday.",
      );
  ...
  function runPerAccount(
    ...
    function getDateRangeYesterdayToToday() {
      ...
      return (
        Utilities.formatDate(yesterdayDate, timeZone, "yyyyMMdd") +
        "," +
        Utilities.formatDate(currentDate, timeZone, "yyyyMMdd")
      );
    }
    ...
    if (foundEntry) {
      Logger.log(
        frequency +
          " -- " +
          accountName +
          " -- Account down -- The Account seems to be active in the last " +
          NUM_HOURS_TO_CHECK +
          " hours.",
      );
    } else {
      Logger.log(
        frequency +
          " -- " +
          accountName +
          " -- The Account seems to be inactive in the last " +
          NUM_HOURS_TO_CHECK +
          " hours.",
      );
      logSheet.appendRow(["Account down", accountId, todayString]);
  ...
  function disapprovedAdsCheck(
    ...
    while (accountIterator.hasNext()) {
      ...
      var whereClause =
        "WHERE ad_group_ad.policy_summary.approval_status = 'DISAPPROVED' AND campaign.status = 'ENABLED' AND campaign.serving_status != 'ENDED' AND ad_group.status = 'ENABLED' AND ad_group_ad.status = 'ENABLED' AND segments.date DURING TODAY";

      var gaqlQuery =
        "SELECT campaign.name, ad_group.name, ad_group_ad.ad.id, ad_group_ad.ad.type, ad_group_ad.policy_summary.approval_status, ad_group_ad.policy_summary.policy_topic_entries FROM ad_group_ad " +
        whereClause;
      ...
      while (results.hasNext()) {
        ...
        Logger.log(
          frequency +
            " -- " +
            accountName +
            " -- Disapproved Ads -- Disapproved Ads found!",
        );
        ...
        mailbodySheet.appendRow([
          ...
          adGroupName,
          " ",
          approvalStatus + ": " + policyFindings,
        ]);
      }

      var gaqlQuery =
        "SELECT asset_group.primary_status_reasons, campaign.name, asset_group.name FROM asset_group WHERE asset_group.primary_status_reasons CONTAINS ANY ('ASSET_GROUP_DISAPPROVED') AND campaign.status = 'ENABLED' AND asset_group.status = 'ENABLED' AND asset_group.primary_status_reasons CONTAINS NONE ('CAMPAIGN_ENDED', 'CAMPAIGN_REMOVED')";
      ...
      while (results.hasNext()) {
        ...
        Logger.log(
          frequency +
            " -- " +
            accountName +
            " -- Disapproved Asset Group -- Disapproved Asset Groups found!",
        );
        logSheet.appendRow(["Disapproved Asset Group", accountId, todayString]);
        mailbodySheet.appendRow([
          accountName,
          "Disapproved Asset Group",
          campaignName,
          adGroupName,
          " ",
          approvalStatus[0],
    ...
    while (accountIterator.hasNext()) {
      ...
      var report = AdsApp.report(
        "SELECT change_event.resource_name, change_event.user_email " +
          "FROM change_event " +
          "WHERE change_event.change_date_time DURING " +
          period +
          " " +
          "AND change_event.user_email NOT REGEXP_MATCH '.*(" +
          userRex +
          ").*' " +
      ...
      while (rows.hasNext()) {
        ...
        Logger.log(
          frequency +
            " -- " +
            accountName +
            " -- Unknown user made a change -- Other user!: " +
            changeUser,
        );
        logSheet.appendRow([
        ...
        mailbodySheet.appendRow([
          ...
          " ",
          " ",
          "An unknown user has made changes to the account: " + changeUser,
        ]);
      }
      addAccountName(accountName);
    }
  ...
  function linkChecker(
    ...
    var linkCheckerLogSheet =
      spreadsheet.getSheetByName("LinkCheckerLog" + consultantName) ||
      spreadsheet.insertSheet("LinkCheckerLog" + consultantName);
    var checkedLinksSheet =
      spreadsheet.getSheetByName("checkedLinks") ||
      spreadsheet.insertSheet("checkedLinks");
    var brokenLinksSheet =
      spreadsheet.getSheetByName("brokenLinks") ||
      spreadsheet.insertSheet("brokenLinks");
    ...
    while (accountIterator.hasNext()) {
      ...
      if (linkCheckerLogSheet.getLastRow() > 1) {
        var range = linkCheckerLogSheet.getRange(
          2,
          1,
          linkCheckerLogSheet.getLastRow() - 1,
          1,
        ); // Start from row 2 to skip header, column 2 for URLs
      ...
      var deduplicatedUrls = Array.from(
        new Set(
          urlValues.filter(function (value) {
      ...
      // check all urls in batches of 200. If error, check the one by one
      var processBatch = [];
      var brokenUrls = [];
      ...
      for (let i = 0; i < deduplicatedUrls.length; i++) {
        ...
        if ((i + 1) % 200 == 0 || i == deduplicatedUrls.length - 1) {
          var timeOut = has45MinutesPassed();
          if (timeOut) break;
          try {
            //Logger.log("🟢 Running fetchAll batch of " + processBatch.length);
            var responses = UrlFetchApp.fetchAll(
              processBatch.map(function (url) {
            ...
            //Logger.log("🔴 Batch failed. Retrying individually...");
            // Fall back naar individuele requests
            for (var x = 0; x < processBatch.length; x++) {
              try {
                var response = UrlFetchApp.fetch(processBatch[x], {
                  muteHttpExceptions: true,
                });
                getStatusCodes(response, processBatch[x]);
              } catch (e) {
                //Logger.log("❌ Failed individually: " + processBatch[x]);
                logBrokenLink(processBatch[x] + " (error: " + e.message + ")");
              }
            }
          }
          processBatch = [];
      ...
      if (brokenUrls.length > 0) {
        ...
        var query = AdsApp.report(
          ...
          FROM ad_group_ad 
          WHERE 
            ad_group_ad.ad.final_urls CONTAINS ANY ('` +
            brokenUrlsString +
            `')
        ...
        var query = AdsApp.report(
          ...
          FROM asset_group 
          WHERE 
            asset_group.final_urls CONTAINS ANY ('` +
            brokenUrlsString +
            `')
        ...
        if (lastRow > 1) {
          ...
          for (var i = values.length - 1; i >= 1; i--) {
            ...
            for (var j = 2; j <= 6; j++) {
              // Column indexes: C=2, D=3, E=4, F=5, G=6
        ...
        if (lastRow > 1) {
          ...
          var accountAssetData = values.map((row) => {
            var error =
              ...
              row[1] +
              " on account level. Url: " +
              row[0]; // Prepend text to URL
            return [
              "Link checker",
              frequency,
              todayString,
        ...
        if (lastRow > 1) {
          ...
          for (var i = values.length - 1; i >= 1; i--) {
            ...
            for (var j = 6; j <= 10; j++) {
              // Column indexes: G=6, H=7, I=8, J=9, K=10
              if (row[j] instanceof Date && row[j] < currentDate) {
                deleteRow = true;
                break; // No need to check further columns if one is already in the past
              }
        ...
        if (lastRow > 1) {
          ...
          for (var i = values.length - 1; i >= 1; i--) {
            ...
            for (var j = 8; j <= 12; j++) {
              // Column indexes: I=8, J=9, K=10, L=11, M=12
              if (row[j] instanceof Date && row[j] < currentDate) {
                deleteRow = true;
                break; // No need to check further columns if one is already in the past
              }
        ...
        if (assetGroupData) {
          linkCheckerLogSheet
            .getRange(
              ...
              assetGroupData[0].length,
            )
            .setValues(assetGroupData);
        }
        if (accountAssetData) {
          linkCheckerLogSheet
            .getRange(
              linkCheckerLogSheet.getLastRow() + 1,
              1,
              accountAssetData.length,
              accountAssetData[0].length,
            )
            .setValues(accountAssetData);
        }
        if (campaignAssetData) {
          linkCheckerLogSheet
            .getRange(
              linkCheckerLogSheet.getLastRow() + 1,
              1,
              campaignAssetData.length,
              campaignAssetData[0].length,
            )
            .setValues(campaignAssetData);
        }
        if (adGroupAssetData) {
          linkCheckerLogSheet
            .getRange(
              linkCheckerLogSheet.getLastRow() + 1,
              1,
              adGroupAssetData.length,
              adGroupAssetData[0].length,
            )
            .setValues(adGroupAssetData);
        }
        ...
        if (lastRow > 1) {
          ...
          // copy the issues
          var range = linkCheckerLogSheet.getRange(
            "A1:G" + linkCheckerLogSheet.getLastRow(),
          );
          var values = range.getValues();
    ...
    function getStatusCodes(response, finalUrl) {
      ...
      if (statusCode != 200) {
        logBrokenLink(finalUrl + " (status: " + statusCode + ")");
      } else {
  ...
  function newConvCheck(
    ...
    while (accountIterator.hasNext()) {
      ...
      // compare conversions
      const noNewConversions = uniqueConversionsYesterday.every(
        (r) => uniqueConversionsHistory.indexOf(r) >= 0,
      );

      // get new conversions
      var newUniqueConversions = uniqueConversionsYesterday.filter(
        (conversion) => !uniqueConversionsHistory.includes(conversion),
      );
      ...
      if (numberOfNewConversions == 0) {
        Logger.log(
          frequency +
            " -- " +
            accountName +
            " -- New conversions found -- No new conversions",
        );
        ...
        Logger.log(
          frequency +
            " -- " +
            accountName +
            " -- New conversions found -- A New conversions",
        );
        logSheet.appendRow(["New conversions found", accountId, todayString]);
        mailbodySheet.appendRow([
          accountName,
          ...
          " ",
          " ",
          "A new conversion is measured: " + newUniqueConversionsAsString,
        ]);
      }
      addAccountName(accountName);
    }
    ...
    while (accountIterator.hasNext()) {
      ...
      var report = AdsApp.report(
        "SELECT campaign.name, campaign.maximize_conversion_value.target_roas, campaign.target_roas.target_roas, " +
          ...
          "FROM campaign WHERE campaign.status = 'ENABLED' AND campaign.experiment_type != 'EXPERIMENT' AND campaign.end_date_time > '" +
          todayString +
          "'",
      );
      ...
      while (rows.hasNext()) {
        ...
        var targetRoasWithNumber = variables.find(
        ...
        if (targetRoasWithNumber < minTRoas) {
          ...
          mailbodySheet.appendRow([
            ...
            " ",
            " ",
            "This campaign has a tROAS of " +
              targetRoasWithNumber +
              "% which is below the minimum tROAS of " +
              minTRoas +
              "%",
          ]);
        }
      ...
      var report = AdsApp.report(
        "SELECT campaign.name, ad_group.name, ad_group.id, campaign.target_roas.target_roas FROM ad_group WHERE campaign.target_roas.target_roas < " +
          minTRoasCorrected +
          " AND campaign.name NOT IN ('" +
          campaignsWithLowTroasAsString +
  ...
  function maxSearchTermCpcCheck(
    frequency,
    accountName,
    logSheet,
    mailbodySheet,
    ...
    while (accountIterator.hasNext()) {
      ...
      var report = AdsApp.report(
        "SELECT campaign.name, ad_group.name, search_term_view.search_term, metrics.average_cpc " +
          "FROM search_term_view WHERE metrics.average_cpc > " +
          maxSearchTermCpc * 1000000 +
          " AND campaign.status = 'ENABLED' " +
      ...
      while (rows.hasNext()) {
        ...
        mailbodySheet.appendRow([
          ...
          adgroupName,
          searchTerm,
          "Yesterday this search term had a CPC of €" +
            cpcCorrected +
            " which is above the maximum CPC of €" +
            maxSearchTermCpc +
            ".",
        ]);
      }
  ...
  function budgetMonitoring(
    ...
    while (accountIterator.hasNext()) {
      ...
      if (campaignType) {
        var whereClause =
          whereClause +
          " AND campaign.advertising_channel_type IN ('PERFORMANCE_MAX', 'SEARCH', 'SHOPPING')";
      }
      ...
      var dayPacingPosition =
        checkNames.indexOf("Day pacing") + columnCorrection;
      var dayPacing = dayOfMonth / getDaysInCurrentMonth;
      settingsSheet.getRange(rowNumber, dayPacingPosition).setValue(dayPacing);

      // get column and set spend pacing
      var spendPacingPosition =
        checkNames.indexOf("Spend pacing") + columnCorrection;
      var spendPacing = spendMtD / accountBudget;
      settingsSheet
        .getRange(rowNumber, spendPacingPosition)
        .setValue(spendPacing);

      var acceptedDeviation = 100 * 0.88 ** (dayOfMonth - 2);

      var spendLine = Math.round(
        (accountBudget / getDaysInCurrentMonth) * dayOfMonth,
      );

      var lowSpendline = Math.round(
        spendLine - spendLine * (acceptedDeviation / 100),
      );
      var highSpendline = Math.round(
        spendLine + spendLine * (acceptedDeviation / 100),
      );
      ...
      if (spendMtD < lowSpendline && !limitType) {
        Logger.log(
          "The spend might be too low. The MtD spend is €" +
            spendMtD +
            ". The spendline is €" +
            spendLine +
            ".",
        ...
        Logger.log(
          "The spend might be too high. The MtD spend is €" +
            spendMtD +
            ". The spendline is €" +
            spendLine,
        );
  ...
  function daysSinceLastConversion(
    frequency,
    accountName,
    logSheet,
    mailbodySheet,
    ...
    while (accountIterator.hasNext()) {
      ...
      var query =
        "SELECT metrics.conversions, segments.date FROM customer WHERE segments.date DURING LAST_30_DAYS ORDER BY segments.date DESC";
        ...
        if (conversions > 0 && daysAgo <= daysToCheck) {
          Logger.log(
            frequency +
              " -- " +
              accountName +
              " -- Conversion recency check -- Last conversions measured on " +
              date +
              ". This was " +
              daysAgo +
              " ago which is within the limit of " +
              daysToCheck +
              " days.",
          );
          ...
          Logger.log(
            frequency +
              " -- " +
              accountName +
              " -- Conversion recency check -- The last conversion was on " +
              date +
              ". This was " +
              daysAgo +
              " ago which is outside the limit of " +
              daysToCheck +
              " days.",
          );
          logSheet.appendRow([
          ...
          Logger.log(
            frequency +
              " -- " +
              accountName +
              " -- Conversion recency check -- No conversions found in the last 30 days.",
          );
          logSheet.appendRow([
            "Conversion recency check",
            accountId,
          ...
          mailbodySheet.appendRow([
            ...
            " ",
            " ",
            "No conversion found in the last 30 days, which is outside the limit of " +
              daysToCheck +
              " days.",
          ]);
          break;
  ...
  function utmTagging(
    ...
    function tryHandleCampaigns(campaignIteratorFunction, campaignType) {
      try {
        ...
        Logger.log(
          "Error processing " + campaignType + " campaigns: " + e.message,
        );
      }
    }
    ...
    function handleCampaigns(campaignIteratorFunction, campaignType) {
      var campaignIterator = campaignIteratorFunction
        ...
        .withCondition('campaign.end_date_time > "' + todayString + '"')
        .get();
      ...
      while (campaignIterator.hasNext()) {
        ...
        try {
          var trackingTemplate =
            ...
            encodeURIComponent(campaign.getName()) +
            "&utm_id=" +
            campaign.getId() +
            "&utm_term={keyword}";
  ...
  function activeAdOrKeyword(
    ...
    while (accountIterator.hasNext()) {
      ...
      var adGroupsWithAds = new Set();
      var adGroupsWithKeywords = new Set();
      var allAdGroups = new Map();
      var allKeywordAdGroups = new Map();
      var rowsToAppend = [];

      // --- 1. Alle actieve ad groups ophalen ---
      var adGroupQuery =
        `
        SELECT campaign.id, ad_group.id, ad_group.name, campaign.name
        FROM ad_group
        ...
        `' 
      `;
      var adGroupRows = AdsApp.search(adGroupQuery);
      while (adGroupRows.hasNext()) {
        var row = adGroupRows.next();
        allAdGroups.set(row.adGroup.id, {
          campaignName: row.campaign.name,
          adGroupName: row.adGroup.name,
        });
      }

      // --- 2. Alle actieve advertenties per ad group ---
      var adQuery = `
        SELECT ad_group.id
        FROM ad_group_ad
        WHERE ad_group_ad.status = 'ENABLED'
          ...
          AND ad_group.status = 'ENABLED'
      `;
      var adRows = AdsApp.search(adQuery);
      while (adRows.hasNext()) {
        var row = adRows.next();
        adGroupsWithAds.add(row.adGroup.id);
      }

      // --- 3. Alle actieve non-DSA ad groups (search only) ---
      var keywordAdGroupQuery =
        `
        SELECT campaign.id, ad_group.id, ad_group.name, campaign.name
        FROM ad_group
        WHERE ad_group.status = 'ENABLED'
        ...
        todayString +
        `'
          AND ad_group.type != 'SEARCH_DYNAMIC_ADS'
      `;
      var keywordAdGroupRows = AdsApp.search(keywordAdGroupQuery);
      while (keywordAdGroupRows.hasNext()) {
        var row = keywordAdGroupRows.next();
        allKeywordAdGroups.set(row.adGroup.id, {
          campaignName: row.campaign.name,
          adGroupName: row.adGroup.name,
        });
      }

      // --- 4. Alle actieve keywords per ad group ---
      var keywordQuery = `
        SELECT ad_group.id
      ...
      `;
      var keywordRows = AdsApp.search(keywordQuery);
      while (keywordRows.hasNext()) {
        var row = keywordRows.next();
        adGroupsWithKeywords.add(row.adGroup.id);
      }

      // --- 5. Controleer ad groups zonder actieve ads ---
      for (var [adGroupId, info] of allAdGroups.entries()) {
        if (!adGroupsWithAds.has(adGroupId)) {
          issues = true;
          rowsToAppend.push([
            accountName,
            "Active ad check",
            ...
            info.adGroupName,
            "",
            "No active ad was found in this ad group.",
          ]);
        }
      }

      // --- 6. Controleer ad groups zonder actieve keywords ---
      for (var [adGroupId, info] of allKeywordAdGroups.entries()) {
        if (!adGroupsWithKeywords.has(adGroupId)) {
          issues = true;
          rowsToAppend.push([
            accountName,
            "Active keyword check",
            info.campaignName,
            info.adGroupName,
            "",
            "No active keyword was found in this ad group.",
          ]);
        }
      }

      // --- 7. Voeg in bulk issues toe aan de sheet ---
      if (rowsToAppend.length > 0) {
        mailbodySheet
          .getRange(mailbodySheet.getLastRow() + 1, 1, rowsToAppend.length, 6)
          .setValues(rowsToAppend);
      }

      // --- 8. Log status als er issues zijn gevonden ---
      if (issues) {
    ...
    while (accountIterator.hasNext()) {
      ...
      if (totalConversions > 0 && convertingConversions > 0) {
        ...
        if (linRodnitzkyRatio < 1.5) {
          performanceMessage =
            "Too conservative. Likely missing incremental, profitable conversions.";
        } else if (linRodnitzkyRatio >= 1.5 && linRodnitzkyRatio < 2.0) {
          performanceMessage =
            "Well-managed. A good mix of consistent winners and growth tests.";
        } else if (linRodnitzkyRatio >= 2.0 && linRodnitzkyRatio < 2.5) {
          performanceMessage =
            "Too aggressive. Wasteful clicks may be due to broad match or poor analysis.";
        } else if (linRodnitzkyRatio >= 2.5) {
          performanceMessage =
            "Mismanaged. Significant wasted spend. Needs immediate optimization.";
        }
        ...
        Logger.log(
          frequency +
            " -- " +
            accountName +
            " -- Lin Rodnitzky Ratio -- The Lin Rodnitzky Ratio " +
            linRodnitzkyRatio.toFixed(2) +
            ".",
        );
  ...
  function merchantCenterIdCheck(
    frequency,
    accountName,
    logSheet,
    mailbodySheet,
      ...
      if (campaignData.hasNext()) {
        ...
        if (merchantId) {
          ...
          mailbodySheet.appendRow([
            ...
            "Merchant Center ID " +
              merchantId +
              " is added to the AdHealth sheet for the account with an acceptable % of disapproved products of 5%.",
          ]);
          Logger.log(
            frequency +
              " -- " +
              accountName +
              " -- Merchant Center ID check -- Merchant ID added: " +
              merchantId,
          );
        }
      } else {
        Logger.log(
          frequency +
            " -- " +
            accountName +
            " -- Merchant Center ID check -- No merchant ID found in performance max campaigns. Checking shopping campaigns",
        );
        // Get Merchent Center ID from performance max campaings
        var query =
          `
          ...
          if (merchantId) {
            ...
            Logger.log(
              frequency +
                " -- " +
                accountName +
                " -- Merchant Center ID check -- No merchant ID found in shopping campaigns either.",
            );
          }
        }
      }
  ...
  function setup() {
    ...
    if (!sheet) {
      ...
      sheet
        .getRange("I5")
        .setValue(
          "Domain or full email address, comma separated: netprofiler.nl,abc@clickvalue.nl",
        );
      ...
      sheet
        .getRange("J4")
        .setValue("netprofiler.nl,leadscope.nl,clickvalue.nl,channable.com");
      sheet.getRange("O3:T3").mergeAcross();
      sheet
        .getRange("O3")
        .setFormula(
          '=if(COUNTIF(O7:O22,true) >0,"Warning! The UTM tagging will overwrite the exsisting UTM tagging on campaing level.","")',
        );
      ...
      var rule1 = SpreadsheetApp.newConditionalFormatRule()
        ...
        .whenFormulaSatisfied(
          "=If(AND(Not(ISBLANK(H7)),ISBLANK(I7)),true,false)",
        )
  ...
  function checkAccount() {
    ...
    var accountIdsInSheet = settingsSheet
      .getRange("A3:A")
      .getValues()
      .flat()
      .filter(Boolean);
    ...
    var accountSelector = AdsManagerApp.accounts().withCondition(
      'LabelNames CONTAINS "' + accountLabel + '"',
    );
    var accountIterator = accountSelector.get();
    ...
    while (accountIterator.hasNext()) {
      ...
      if (accountIdsInSheet.indexOf(accountId) === -1) {
        // Find the first empty row after row 2 and add the account ID and name.
        var emptyRow =
          settingsSheet
            .getRange("A3:A")
            .getValues()
            .findIndex((row) => !row[0]) + 3;
        settingsSheet.getRange("A" + emptyRow).setValue(accountId);
        settingsSheet.getRange("B" + emptyRow).setValue(accountName);
        settingsSheet.getRange("M" + emptyRow).setValue(100);
        settingsSheet.getRange("O" + emptyRow).setValue(20);
        settingsSheet.getRange("Y" + emptyRow).setValue(1);

        var columns = ["E", "F", "G", "H", "I", "L", "N", "P", "R", "X", "Z"];
        for (var i = 0; i < columns.length; i++) {
          settingsSheet
            .getRange(columns[i] + emptyRow)
            .setDataValidation(
          ...
          if (columns[i] === "N" || columns[i] === "P" || columns[i] === "X") {
        ...
        settingsSheet
          .getRange("C3")
          .copyTo(
            settingsSheet.getRange("C" + emptyRow),
            SpreadsheetApp.CopyPasteType.PASTE_DATA_VALIDATION,
            false,
          );
        settingsSheet.getRange("C" + emptyRow).setValue(consultantName);
        ...
        // Account bestaat al → consultantName toevoegen indien nodig.

        // Vind de rij van dit account (index + 3 omdat data vanaf A3 start)
        var rowIndex = accountIdsInSheet.indexOf(accountId) + 3;

        // Haal huidige waarde uit kolom C
        var currentValue = settingsSheet.getRange("C" + rowIndex).getValue();

        // Controleer of consultantName al aanwezig is
        var consultants = currentValue
          .toString()
          .split(",")
          .map(function (s) {
            return s.trim();
        ...
        if (consultants.indexOf(consultantName) === -1) {
          // Consultant toevoegen
          consultants.push(consultantName);

          // Nieuwe comma-separated string maken
          var newValue = consultants.join(", ");

          // Wegschrijven naar kolom C
          settingsSheet.getRange("C" + rowIndex).setValue(newValue);
        }
      }
    }
  }
  ...
  function getHour() {
    ...
    var amsterdamDate = Utilities.formatDate(
      new Date(),
      amsterdamTimeZone,
      "yyyy-MM-dd HH:mm:ss",
    );
  ...
  function clearLogs(frequency) {
    var logsSheetName = frequency + "Log" + consultantName;
    ...
    if (logsSheet) {
      ...
      Logger.log(
        logsSheetName + " has been clear from row 2 untill row " + logsLastRow,
      );
    }
  }
  ...
  function sendEmail(mailadres) {
    var mainMailBodySheet = spreadsheet.getSheetByName(
      "MailBody" + consultantName,
    );
    ...
    if (tableRows.length > 0) {
      ...
      var emailBody =
        ...
        <span>Check your settings <a href="` +
        shortUrl +
        `">in this sheet</a>`;
      ...
      */
      ...
      // Open the log sheet
      var logSpreadsheet = SpreadsheetApp.openByUrl(
        "https://docs.google.com/spreadsheets/d/1lfGUi41SSZemcxMdhkchwe8ocTo77xDiaUYrlHn7EiQ/edit#gid=0",
      );
  ...
  function logAccounts(today) {
    ...
    if (dataToWrite.length > 0) {
      accountLogsSheet
        .getRange(lastRow + 1, 1, dataToWrite.length, 3)
  ...
  function addAccountName(accountName) {
    ...
    if (flatColumnA.indexOf(accountName) === -1) {
      ...
      var firstEmptyRowIndex =
        flatColumnA.findIndex(function (value) {
          return value == "";
        }) + 1;
      ...
      AccountLogSheet.getRange("B" + firstEmptyRowIndex).setValue(
        "Main script",
      );
    ...
    for (let i = 0; i < accountNames.length; i++) {
      ...
      if (budget) {
        mailbodySheet.appendRow([
          ...
          "",
          " ",
          "The monthly budget for this account is set to €" +
            budget +
  ...
  function setupMailBodySheet(consultantName) {
    var sheetName = "MailBody" + consultantName;

    // Controleer of de sheet bestaat
    var mailbodySheet = spreadsheet.getSheetByName(sheetName);
    if (!mailbodySheet) {
      // Sheet bestaat niet, maak aan
      mailbodySheet = spreadsheet.insertSheet(sheetName);
      ...
      mailbodySheet.getRange(1, 1, 1, headers.length).setValues([headers]);

      // Eventueel opmaak toevoegen
      mailbodySheet.getRange("A1:F1").setFontWeight("bold");
      mailbodySheet.setFrozenRows(1);
    }
    return mailbodySheet;
  }

  function getOrCreateLogSheet(frequency, consultantName) {
    var name = frequency + "Log" + consultantName;
    var sheet =
      spreadsheet.getSheetByName(name) || spreadsheet.insertSheet(name);
    sheet.hideSheet();
    if (sheet.getLastRow() === 0)
      sheet.appendRow(["Script", "Account ID", "Date"]);
    return sheet;
  }
